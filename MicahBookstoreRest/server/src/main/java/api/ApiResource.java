...
public class ApiResource {
    ...
    public List<Book> suggestedBooks(@PathParam("category-id") long categoryId,
        ...
        try {
            ...
            throw new ApiException("Suggested-books lookup by category-id %d failed", e);
        }
    }
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Book> booksByCategoryId(@PathParam("category-name") String name,
                                        @Context HttpServletRequest httpRequest) {
        try {
    ...
    @Produces(MediaType.APPLICATION_JSON)
    public List<Book> suggestedBooks(@PathParam("category-name") String name,
                                     @QueryParam("limit") @DefaultValue("3") int limit,
                                     @Context HttpServletRequest request) {
        try {
            Category category = categoryDao.findByName(name);
